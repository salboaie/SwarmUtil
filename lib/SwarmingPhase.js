...
function getWaitingContext(swarm,desiredPhaseName, nodeHint, timeOut, retryTimes, phaseExecutionId){
    return function(){
        ...
        if(ctxt.confirmedExecution == true){
            ...
            //cprint("Failed " + phaseExecutionId);
            if(retryTimes == 0){
                this.sendFail();
            }else{
                this.safeSwarm(desiredPhaseName, timeOut,nodeHint,retryTimes-1,false);
            }
        }
        endExecutionContext();
    }.bind(swarm);
...
SwarmingPhase.prototype.safeSwarm = function (phaseName, nodeHint,timeOut,retryTimes,persistent) {
    ...
    //cprint("New phaseid " + this.meta.phaseExecutionId);
    this.meta.confirmationNode = thisAdapter.nodeName;
    this.meta.pleaseConfirm = true;

    setTimeout(getWaitingContext(this, phaseName, nodeHint, timeOut, retryTimes, this.meta.phaseExecutionId), timeOut);
