...
var redisPS = require("./RedisPubSub.js");
var udpPS = require("./UdpPubSub.js");
/**
 * Adapter core class
 * @param nodeName
 * @constructor
...
exports.init = function (nodeName, onReadyCallback, subCallBackSink, verbose) {
    ...
    thisAdapter.useUDP              = thisAdapter.config.Core.communication == "udp";
    ...
    if(thisAdapter.useUDP){
        if(thisAdapter.nodeName == "Core"){
            pubsubRedisClient              = udpPS.createUdpCoreNode(
                thisAdapter.config.Core.current,
                thisAdapter.config.Core.cores);
        } else{
            pubsubRedisClient              = udpPS.createUdpPubSubNode(thisAdapter.config.Core.cores);
        }
        ...
        pubsubRedisClient.on("error",onRedisError);
        pubsubRedisClient.on("reconnecting",onRedisReconnecting);    */
        pubsubRedisClient              = redisPS.createRedisPubSubClient(thisAdapter.redisPort, thisAdapter.redisHost);
        pubsubRedisClient.on("error",onRedisError);
        pubsubRedisClient.on("reconnecting",onRedisReconnecting);
    }
    ...
    pubsubRedisClient.on("message", function (channel, initVars) {
