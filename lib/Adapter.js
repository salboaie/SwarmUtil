...
function AdapterBase(nodeName) {
    ...
    this.renameSession = function( oldSession, newSession){
        var outlet = sessionOutlets[oldSession];
        sessionOutlets[newSession] = outlet;
    ...
    this.addOutlet = function(outlet){
        ...
        sessionOutlets[outlet.getSessionId()]  = outlet;
    }

    this.deleteOutlet = function(outlet){
        delete connectedOutlets[outlet.getOutletId()];
       delete sessionOutlets[outlet.getSessionId()];
    }
}
        ...
        if(!util.isGroupChannelName(channel)){
            ...
            if(!thisAdapter.isSleeping || thisAdapter.onSleepCanExecuteCallback(initVars)){
                if(initVars.meta.toUserRequest){
                    outlet = thisAdapter.findOutletForSession(initVars.meta.sessionId);
                    if(!outlet) {
