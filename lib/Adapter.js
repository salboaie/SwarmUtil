...
SwarmingPhase.prototype.swarm = function (phaseName, nodeHint ) {
    ...
    try{
        ...
        if (nodeHint == undefined) {
            var phase = thisAdapter.compiledSwarmingDescriptions[this.swarmingName][phaseName];
            if(phase == undefined){
                logErr("Undefined phase " + phaseName + " in swarm " + this.swarmingName);
                return;
            }
        ...
        if(this.debug == true || thisAdapter.verbose == true){
            cprint("Starting swarm "+this.swarmingName +  " towards " + targetNodeName + ", Phase: "+ phaseName);
        }
...
SwarmingPhase.prototype.confirmExecution = function() {
    var phase = this.meta.onSuccesPhase;
    if( phase == undefined){
        phase = "onSuccesPhase";
    }
    this.swarm(phase);
...
SwarmingPhase.prototype.confirmFail = function() {
    var phase = this.meta.onErrorPhase;
    if( phase == undefined){
        phase = "onErrorPhase";
    }
    this.swarm(phase);
...
SwarmingPhase.prototype.safeSwarm = function (phaseName, timeOut,nodeHint,retryTimes,save) {
    ...
    this.meta.fromNode = thisAdapter.nodeName;
    this.meta.confirmExecution = true;
    ...
    setTimeout(function(){
        if(ctxt.confirmedExecution == true){
            ...
            if(retryTimes == 0){
                ...
                this.safeSwarm(phaseName, timeOut,nodeHint,retryTimes,save);
            }
        }
    }.bind(this), timeOut);
...
function onMessageFromQueue(initVars) {
    ...
    try{
        if (swarmingPhase.debug == true || thisAdapter.verbose == true) {
            cprint("[" + thisAdapter.nodeName + "] received message: " + J(initVars));
        }
    ...
    if(reportSucces == true){
        startSwarm("ConfirmExecution",swarmingPhase);
    }
}
