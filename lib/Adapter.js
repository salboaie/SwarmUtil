...
exports.init = function (nodeName, onReadyCallback,onSleepCanExecuteCallback) {
    ...
    redisClient.on("connect",onRedisReady);
    ...
    pubsubRedisClient.on("subscribe",onRedisReady);
...
function mkUri(type, value) {
    return  thisAdapter.coreId + "::" + type + "/" + value;
}
...
SwarmingPhase.prototype.setTimeout = function (timeOut,phaseName, nodeHint) {
    try{
        var targetNodeName = nodeHint;
        if (nodeHint == undefined) {
            targetNodeName = thisAdapter.compiledSwarmingDescriptions[this.swarmingName][phaseName].node;
        ...
        if(nodeHint == thisAdapter.nodeName ){
            ...
            this.__internal__timeOut   = timeOut;
            this.__internal__nextPhase = phaseName;
            this.swarm("__internal_setTimeout",nodeHint);
        }
    }
    catch(err){
