...
function AdapterBase(nodeName) {
    ...
    this.findOutlet = function(sessionId){
        return connectedOutlets[sessionId];
    }

    this.addOutlet = function(sessionId, outlet){
        connectedOutlets[sessionId] = outlet;
    }

    this.deleteOutlet = function(sessionId){
        delete connectedOutlets[sessionId];
    }
}
    ...
    pubsubRedisClient.on("message", function (channel, message) {
        ...
        if(!util.isGroupChannelName(channel)){
            ...
            if(!thisAdapter.isSleeping || thisAdapter.onSleepCanExecuteCallback(initVars)){
                if(initVars.meta.honeyRequest){
                    var outlet = thisAdapter.findOutlet(initVars.meta.sessionId);
                    if(outlet == null){
                        logErr("Can't send home a swarm belonging to session " + initVars.meta.sessionId + "\nDump: "+ message );
                    } else {
