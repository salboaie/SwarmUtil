        ...
        if(!util.isGroupChannelName(channel)){
            ...
            if(!thisAdapter.isSleeping || thisAdapter.onSleepCanExecuteCallback(initVars)){
                if(initVars.meta.honeyRequest){
                    var outlet = thisAdapter.connectedOutlets[initVars.meta.sessionId];
                    if(outlet == null){
                        logErr("Can't sent home a swarm belonging to session " + initVars.meta.sessionId + "\nDump: "+ message) );
                    } else {
...
function onRedisError(event){
    cprint("Redis connection error! Please start redis server or check your configurations!" + event.stack);
}
